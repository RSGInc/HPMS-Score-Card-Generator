...
create_page_summary <- function(
  ...
  #browser()
  quality_results = fread(paste0("data/",gState_Labels[index==state,label],"/",gState_Labels[index==state,label],"_",year,"_",year_compare,"_quality_summary.csv"))
  
  add_summary_col_labels(
    paste0(gVariables[x1,Label],
           if(ramps){""}else{quality_results[Item_Number==gVariables[x1,Item_Number],paste0(" (o:",Outlier_Score,"/a:",Adjacency_Score,"/y:",YOY_Score,")")]}),1
  )
  
  if(show2){
    add_summary_col_labels(
    paste0(gVariables[x2,Label],
           if(ramps){""}else{quality_results[Item_Number==gVariables[x2,Item_Number],paste0(" (o:",Outlier_Score,"/a:",Adjacency_Score,"/y:",YOY_Score,")")]}),2
    )
  }
  ...
  if(show3){
    ...
    paste0(gVariables[x3,Label],
           if(ramps){""}else{quality_results[Item_Number==gVariables[x3,Item_Number],paste0(" (o:",Outlier_Score,"/a:",Adjacency_Score,"/y:",YOY_Score,")")]}),3
    )
  }
