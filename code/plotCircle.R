           ...
           R)
  {
    ...
    if (nrow(data[year_record == year & data_item == variable]) == 0)
    {
      ...
      if (recipe == 1)
      {
        ...
        outliers[, type := type + 
                   1 * (as.numeric(perc_miles) < tol_Med) + 
                   1 * (as.numeric(perc_miles) < tol_High)]
      ...
      if (recipe == 2)
      {
        var.1    <- data[year_record == year & data_item == variable,
                         list(route_id,
                              ...
                              F_SYSTEM)]
        var.2    <-
          data[year_record == year_compare & data_item == variable,
               list(route_id,
                    begin_point,
                    end_point,
                    value_numeric,
        ...
        report <-
          var.yoy[, bin2 := factor(
            1 + as.numeric(value_numeric.x != value_numeric.y),
            levels = c(1, 2),
            labels = c("No Change", "Changed")
          )]
        
        report <-
          report[, sum(end_point.x - begin_point.x), by = .(bin2)]
        ...
        {
          quality.value <-
            report[as.numeric(bin2) == 1 | is.na(bin2), sum(V1)] * 100
        }
        if (expectedChange == "N")
        {
          quality.value <-
            report[as.numeric(bin2) == 2 | is.na(bin2), sum(V1)] * 100
        }
        type <-
          type + as.numeric(quality.value < tol_Med) + as.numeric(quality.value < tol_High)
      }
    }
