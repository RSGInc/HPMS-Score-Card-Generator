...
for ( i in nrow(rename_tbl) ){
  ...
  sql = str_glue(
    ...
    "\twhere (Year_Record = 2020 or Year_Record = 2019) and Data_Item = '{name_8}'")
...
section_tbl %>%
  filter(Data_Item == !!rename_tbl$name_8[6]) %>%
  count(Year_Record, State_Code) %>%
  count(Year_Record)
  
section_tbl %>%
  filter(Data_Item == !!rename_tbl$name_9[6]) %>%
  count(Year_Record, State_Code) %>%
  count(Year_Record)
