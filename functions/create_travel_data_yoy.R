...
create_travel_data_yoy <- function(
  ...
  if(type == 'numeric'){
    if(ramps){
      var.1 = data[stateid == state & datayear == year &
                   ...
                   list(routeid, beginpoint, end_point,
  ...
  if(type == 'text'){
    if(ramps){
      var.1 = data[stateid == state & datayear == year &
                   ...
                   list(routeid,beginpoint,end_point,value.1 = value_text,F_SYSTEM)]
      
      var.2 = data[stateid == state&datayear == yearcomparison &
                     data_item == variable & FACILITY_TYPE == 4 & !is.na(value_text),
                   list(routeid,beginpoint,end_point,value.2 = value_text,F_SYSTEM)]
  ...
  if(nrow(var.yoy) > 0){ # we have something to report
    ...
    if(histtype == 1){
      ...
      totalmiles <- report[, sum(end_point - beginpoint)]
      ...
      report <- report[, .(V1 = sum(end_point - beginpoint)), by=.(color)]
    ...
    # custom axis labels
    if(histtype == 1){
      p <- ggplot(report, aes(x=bin2,fill=color,weight=end_point-beginpoint)) +
        geom_bar(width=0.75)
