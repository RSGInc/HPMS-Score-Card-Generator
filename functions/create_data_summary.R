...
create_data_summary <- function(data, state, year, year_compare){
  ...
  idx_st_fsystem <- with(data, (stateid == state & dataitem == 'F_SYSTEM'))
  ...
  # Centerline
  n_CtrLine.1 <- data[IDX & datayear == year,         sum(endpoint-beginpoint,na.rm = TRUE),]
  n_CtrLine.2 <- data[IDX & datayear == year_compare, sum(endpoint-beginpoint,na.rm = TRUE),]

  # Lane miles
  n_LaneMiles.1 <- data[IDX & datayear==year,
                        sum(THROUGH_LANES * (endpoint - beginpoint), na.rm = TRUE), ]
  
  n_LaneMiles.2 <- data[IDX & datayear == year_compare,
                        sum(THROUGH_LANES * (endpoint - beginpoint), na.rm = TRUE), ]

  # Data items
  n_Variables.1 <- length(data[stateid == state & datayear == year, unique(dataitem), ])
  n_Variables.2 <- length(data[stateid == state & datayear == year_compare, unique(dataitem), ])
  ...
  dt_sections.1 <- data[IDX & datayear == year,
                       list(routeid, beginpoint, endpoint, num_sections)]
  ...
  dt_sections_j = dt_sections.2[dt_sections.1,on=.(routeid,beginpoint,endpoint)]
