...
getYOY <- function(data, year, yearcomparison, variable, yoy_change){
  ...
  var.1    <- data[datayear == year & data_item==variable,
                   list(routeid, begin_point, end_point, value_numeric, value_text, value_date,
  ...
  #            list(routeid, begin_point, end_point, value_numeric, F_SYSTEM)][
  ...
  # 
  # # Miles by routeid
  # r1 <- var.1[F_SYSTEM == 1, .(miles = sum(end_point - begin_point)), by=list(routeid)]
  # r2 <- data[datayear == yearcomparison & data_item==variable & F_SYSTEM == 1,
  #            list(routeid, begin_point, end_point, value_numeric, F_SYSTEM)][
  #              , .(miles = sum(end_point - begin_point)), by=list(routeid)]
