...
create_overall_condition <- function(data, state, year, population)
{
     ...
     rutting  <- data[state_code  ==  state & datayear == year & data_item == "RUTTING" & FACILITY_TYPE!=4,
                      list(route_id, begin_point, end_point, value_numeric), ]
     setnames(rutting, "value_numeric", "rutting")

     iri      <- data[state_code == state & datayear == year & data_item == "IRI" & FACILITY_TYPE!=4,
                      list(route_id, begin_point, end_point, value_numeric), ]
     setnames(iri, "value_numeric", "iri")
     
     faulting <- data[state_code == state & datayear == year & data_item == "FAULTING" & FACILITY_TYPE!=4,
                      list(route_id, begin_point, end_point, value_numeric), ]
     setnames(faulting, "value_numeric", "faulting")
     
     cracking <- data[state_code == state & datayear == year & data_item == "CRACKING_PERCENT" & FACILITY_TYPE!=4,
                      list(route_id, begin_point, end_point, value_numeric), ]
     setnames(cracking, "value_numeric", "cracking")
     
     surface <- data[state_code == state & datayear == year & data_item == "SURFACE_TYPE" & FACILITY_TYPE!=4,
                     list(route_id, F_SYSTEM, Interstate, NHS, begin_point, end_point, value_numeric), ]
     setnames(surface, "value_numeric", "surface")
     
     urban <- data[state_code == state & datayear == year & data_item == "URBAN_ID" & FACILITY_TYPE!=4,
                   list(route_id, begin_point, end_point, value_numeric), ]
